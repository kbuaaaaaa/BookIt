...
class Admin_Manage_Window(QWidget):
    # class constructor
    def __init__(self,stack_widget):
        # call QWidget constructor
        super().__init__()
        self.__stack = stack_widget
        self.ui = Ui_Admin_Manage.Ui_Form()
    ...
    def displayBookings(self):
        try:
            ...
            print(type(bookings))
            print(bookingList)
            print(type(bookingList))
            all_list = []
            for books in bookingList:
                all_list = all_list+books
            for booking in all_list:
                # print(booking)
    ...
    def book(self):
        ...
        #find index for booking array
        i=0
        j=0
        for user_book in bookingList:
            j+=1
            if(len(user_book)==0):
                continue
            k=0
            for book in user_book:
                ...
                i+=1
                k+=1
        print(i,j,k,select_row)
        print(user_no,book_no)
        print("break1")
        if self.ui.book_btn.text() == 'Change':
            ...
            if not self.isTimeOverLap(select_booking,root):
                ...
                self.gen_QR(key)
                flag = True
                bookingList[user_no][book_no] = select_booking
            else:
            ...
            bookingList[user_no].pop(book_no)
        count = 0
        for user,_ in item:
            root[user] = bookingList[count]
            count +=1
...
if __name__ == '__main__':
    ...
    mainWindow = Admin_Manage_Window(stack)
    mainWindow.show()
